...
// Audit hook: create PortfolioChange entries on create/update
try {
  const PortfolioChange = require("./PortfolioChange");

  // After save (create or update)
  portfolioSchema.post("save", async function (doc) {
    try {
      const changeType = doc.isNew ? "mint" : "update";
      const userId = doc.updatedBy || doc.createdBy || null;
      await PortfolioChange.create({
        userId: userId,
        tokenId: doc.blockchainId || doc.transactionHash || null,
        changeType,
        diff: { doc },
        meta: { source: "portfolio_model_post_save" },
      });
    } catch (err) {
      // Do not block save on logging failure
      // eslint-disable-next-line no-console
      console.error("Failed to create PortfolioChange on save:", err.message);
    }
  });
  ...
  portfolioSchema.post("findOneAndUpdate", async function (doc) {
    try {
      ...
      await PortfolioChange.create({
        ...
        changeType: "update",
        diff: { doc },
        meta: { source: "portfolio_model_findOneAndUpdate" },
      });
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(
        "Failed to create PortfolioChange on findOneAndUpdate:",
        err.message
      );
    }
  });
} catch (err) {
  // If PortfolioChange cannot be required (startup ordering), skip hook
  // eslint-disable-next-line no-console
  console.warn("PortfolioChange hook not registered:", err.message);
}
