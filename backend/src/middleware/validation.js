...
const marketplaceItemCreate = Joi.object({
  ...
  category: Joi.string()
    .valid("reward", "merchandise", "service", "other")
    .required(),
...
const adminUpdateUserRole = Joi.object({
  role: Joi.string()
    .valid("student", "institution", "admin", "super_admin")
    .required(),
});
...
const adminUpdateUser = Joi.object({
  ...
  academicInfo: Joi.object({
    ...
    totalCredits: Joi.number().min(0).optional(),
    completedCredits: Joi.number().min(0).optional(),
    ...
    courses: Joi.array()
      .items(
        Joi.object({
          courseCode: Joi.string().max(20).optional(),
